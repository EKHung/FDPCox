...
preprocess <- function(obs, cutoff=1){
    ...
    if (is.na(cutoff)){
        ...
        cutoff_ind <- findInterval(cutoff, sort_obs[, 1])
...
cdp_tree <- function(data, beta_hat, epsilon, levels, truncation, delta=0.001, 
    ...
    breslow_beta <- list('time'=rep(0, timepoints+1), 'breslow'=rep(0, timepoints+1))
    breslow_beta$breslow[1] = 0
    for (u in 2:timepoints){
    ...
    for (m in 1:(2^levels)){
        while(breslow_beta$time[counter] < cutoff*m/(2^levels) && counter<timepoints){
            counter <- counter + 1
        }
...
fdp_trees_to_breslow <- function(trees_list, weights, cutoff=1){
    S <- length(trees_list)
    height <- dim(trees_list[[1]])[1]; bins <- dim(trees_list[[1]])[2]
    timepoints <- cutoff*(1:bins)/bins
    steps <- rep(0, bins)
    for(t in 1:bins){
